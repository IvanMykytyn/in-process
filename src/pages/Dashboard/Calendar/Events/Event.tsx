...
const Event: FC<ExtendedSingleBooking> = memo((eventData) => {
  ...
  const showEvent =
    eventData.creator.email === user?.email ||
    user?.role === "admin" ||
    user?.role === "user";
    ...
    <>
      ...
      >
        <div
          className={cn(
            scss["event"],
            !showEvent ? scss["private-event"] : null
          )}
        >
          <div
          ...
          />
          {eventData &&
            buildEventContentDay(currentEventHeight, eventData, showEvent)}
        </div>
      </div>
    </>
  );
  ...
  if (isListViewType) {
    return (
      <div className={scss["event-calendar-content-list"]}>
        {!showEvent && (
          <img className={scss["event_lock-icon"]} src={lock} alt='lock' />
        )}
        <h3 className={scss["event__month-title"]}>{name}</h3>
      </div>
    );
  ...
  if (eventHeight <= 32) {
    return (
      <div data-size={"small"}>
        ...
        {isWeekViewType && !showEvent && (
          <img
            className={scss["event_lock-icon"]}
            src={lock}
            alt='lock'
            style={{ opacity: 0.6 }}
          />
...
const getClockContent = (
  ...
  return (
    <div
      ...
      style={{ paddingRight: !showEvent ? 15 : undefined }}
    >
      {!showEvent && (
        <img className={scss["event_lock-icon"]} src={lock} alt='lock' />
      )}
